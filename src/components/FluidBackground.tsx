...
const FluidBackground = () => {
  ...
  `
    ...
    void main() {
      ...
      // Add fluid motion using Noise
      float noiseVal = snoise(vec3(uv.x * 2.0 + uMouse.x * 0.1, uv.y * 2.0 + uMouse.y * 0.1, uTime * 0.2));
      
      // Secondary noise layer for detail
      float noiseVal2 = snoise(vec3(uv.x * 4.0 - uTime * 0.1, uv.y * 4.0 + uTime * 0.1, uTime * 0.1));

      // Combine noises
      float mixedNoise = (noiseVal + noiseVal2 * 0.5);

      // Color Mixing
      // Base dark color
      vec3 darkBase = vec3(0.02, 0.02, 0.05);
      
      // Accent highlights (Electric Blue/Purple) based on noise peaks
      vec3 accent1 = vec3(0.2, 0.4, 1.0); // Blue
      vec3 accent2 = vec3(0.6, 0.2, 0.8); // Purple

      // Mix
      vec3 finalColor = mix(darkBase, accent1, smoothstep(-0.2, 0.6, mixedNoise));
      finalColor = mix(finalColor, accent2, smoothstep(0.4, 1.0, mixedNoise));
